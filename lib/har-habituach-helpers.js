...
async function updatePremiumSummary(person, ownerId) {
    ...
    // Premium summary fields exist only on insured (object 2), not on lead.
    if (person.personType === "insured") {
        await putRequest(`/record/2/${person.insuredId}`, {
    ...
    // Stamp lead with last-upload tracking (agent + datetime) — runs whenever a leadId exists.
    if (person.leadId) {
        const stampFields = { pcfsystemfield576: new Date().toISOString() };
        if (ownerId) stampFields.pcfsystemfield575 = ownerId;
        await putRequest(`/record/1003/${person.leadId}`, stampFields);
    }
}
