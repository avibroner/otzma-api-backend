...
async function createInsuranceRecord(row, person, fieldOptions, bufferMapping, ownerId) {
    ...
    let warning;
    const hasBufferMapping = Object.keys(bufferMapping).length > 0;
    if (row.secondaryBranch && !branchId) {
        warning = `ענף משני "${row.secondaryBranch}" לא נמצא בפיירברי`;
    } else if (hasBufferMapping && row.secondaryBranch && !bufferId) {
...
async function updatePremiumSummary(person, ownerId) {
    const queryStr = person.personType === "insured"
        ? `(pcfsystemfield139 = ${person.insuredId})`
        : `(pcfsystemfield223 = ${person.leadId})`;
    ...
    const sumRes = await putRequest(`/record/${objecttype}/${objectid}`, fields);
    console.log(`[har-habituach] premium-summary PUT /record/${objecttype}/${objectid} response:`, JSON.stringify(sumRes));

    // Stamp lead with last-upload tracking (separate PUT to avoid coupling with summary fields).
    // pcfsystemfield230+ are currently invalid on leads — see Fireberry response above.
    if (person.leadId) {
        const stampFields = { pcfsystemfield576: new Date().toISOString() };
        if (ownerId) stampFields.pcfsystemfield575 = ownerId;

        const stampRes = await putRequest(`/record/1003/${person.leadId}`, stampFields);
        console.log(`[har-habituach] lead-stamp PUT /record/1003/${person.leadId} fields:`, JSON.stringify(stampFields), "response:", JSON.stringify(stampRes));
    }
}
...
async function notifyUnmappedBranches(unmappedRecords, idNumber, personType, totalRows, createdCount) {
    const webhookUrl = process.env.MAKE_ALERT_WEBHOOK_URL || "https://hook.eu1.make.com/pfpkkscb52w7qp1xk19qp7aoq8rerdlq";
