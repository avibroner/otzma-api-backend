...
async function updatePremiumSummary(person, ownerId) {
    ...
    if (person.personType === "insured") {
        ...
        fields = {
            ...
            pcfsystemfield576: new Date().toISOString(),
        };
        if (ownerId) fields.pcfsystemfield575 = ownerId;
    }

    console.log(`[har-habituach] PUT /record/${objecttype}/${objectid} fields:`, JSON.stringify(fields));
    const putRes = await putRequest(`/record/${objecttype}/${objectid}`, fields);
    console.log(`[har-habituach] PUT response:`, JSON.stringify(putRes));
}
